...
DQN, DDPG, A2C, and PPO train through a shared EnvPool vector-environment interface.
...
- `checkpoints/last.pt`: the latest resumable DQN/DDPG/A2C/PPO checkpoint
- `checkpoints/best.pt`: the best DQN/DDPG/A2C/PPO checkpoint by evaluation mean return
  after at least one evaluation has run
- `checkpoints/step_<step>.pt`: optional DQN/DDPG/A2C/PPO periodic checkpoints when
...
DQN, DDPG, A2C, and PPO resume from a run directory by loading `checkpoints/last.pt`:
...
Evaluate a saved DQN/DDPG/A2C/PPO checkpoint:
...
Vectorized DQN/DDPG/A2C/PPO runs can override `env.num_envs`:

```sh
uv run python src/train_a2c.py --config configs/cartpole_a2c.yaml --set env.num_envs=4
```

DQN/DDPG exploration settings live under `train.exploration`:
